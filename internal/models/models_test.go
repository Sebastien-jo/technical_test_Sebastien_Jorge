...
func TestNewTokenBucket_StartsFull(t *testing.T) {
	...
	assert.Equal(t, int64(100), tb.Capacity)
...
func TestTokenBucket_Allow_ConsumesTokens(t *testing.T) {
	...
	allowed, remaining, retryAfter := tb.Allow(3)
	assert.True(t, allowed)
	assert.Equal(t, int64(7), remaining)
	assert.Zero(t, retryAfter)
}

func TestTokenBucket_Allow_DeniesWhenEmpty(t *testing.T) {
...
func TestTokenBucket_Allow_ExactExhaustion(t *testing.T) {
	tb := NewTokenBucket(10, time.Minute)
...
func TestTokenBucket_GetRemaining_ReflectsConsumption(t *testing.T) {
	tb := NewTokenBucket(20, time.Minute)
	tb.Allow(5)
	assert.Equal(t, int64(15), tb.GetRemaining())
}

func TestTokenBucket_GetResetTime_FullBucketIsNow(t *testing.T) {
	tb := NewTokenBucket(10, time.Minute)
	reset := tb.GetResetTime()
	assert.WithinDuration(t, time.Now(), reset, 50*time.Millisecond)
}

func TestTokenBucket_GetResetTime_AfterExhaustion(t *testing.T) {
	tb := NewTokenBucket(60, time.Minute) // 1 token/s
	tb.Allow(60)
	reset := tb.GetResetTime()
	assert.WithinDuration(t, time.Now().Add(time.Minute), reset, 2*time.Second)
}

// ── QuotaTracker.Allow ────────────────────────────────────────────────────────

func TestQuotaTracker_Allow_DelegatesToBucket(t *testing.T) {
	qt := &QuotaTracker{
		Bucket:    NewTokenBucket(5, time.Minute),
		CreatedAt: time.Now(),
	}

	allowed, remaining, _ := qt.Allow(3)
	assert.True(t, allowed)
	assert.Equal(t, int64(2), remaining)
	assert.False(t, qt.UpdatedAt.IsZero())
}

func TestQuotaTracker_Allow_DeniesWhenBucketEmpty(t *testing.T) {
	qt := &QuotaTracker{Bucket: NewTokenBucket(1, time.Minute), CreatedAt: time.Now()}
	qt.Allow(1)

	allowed, _, _ := qt.Allow(1)
	assert.False(t, allowed)
}
